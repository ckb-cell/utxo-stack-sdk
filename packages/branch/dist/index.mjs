...
var payloadToAddress = (payload, isMainnet = true) =>
  bech32m.encode(isMainnet ? 'ckb' /* Mainnet */ : 'ckt' /* Testnet */, bech32m.toWords(payload), MAX_BECH32_LIMIT)
...
var serializeEpoch = ({ length, index, number }) => {
  ...
  const epochSince = JSBI.add(
    JSBI.add(
      JSBI.add(JSBI.leftShift(JSBI.BigInt(32), JSBI.BigInt(56)), JSBI.leftShift(JSBI.BigInt(length), JSBI.BigInt(40))),
...
var parseEpoch = epoch => ({
  length: `0x${JSBI.bitwiseAnd(JSBI.signedRightShift(JSBI.BigInt(epoch), JSBI.BigInt(40)), JSBI.BigInt(65535)).toString(
    16,
  )}`,
  index: `0x${JSBI.bitwiseAnd(JSBI.signedRightShift(JSBI.BigInt(epoch), JSBI.BigInt(24)), JSBI.BigInt(65535)).toString(
    16,
  )}`,
  number: `0x${JSBI.bitwiseAnd(JSBI.BigInt(epoch), JSBI.BigInt(16777215)).toString(16)}`,
})
...
var calculateTransactionFee = (transactionSize, feeRate) => {
  ...
  const ratio = JSBI2.BigInt(1e3)
  const base = JSBI2.multiply(JSBI2.BigInt(`${transactionSize}`), JSBI2.BigInt(`${feeRate}`))
  const fee = JSBI2.divide(base, ratio)
  if (JSBI2.lessThan(JSBI2.multiply(fee, ratio), base)) {
    return `0x${JSBI2.add(fee, JSBI2.BigInt(1)).toString(16)}`
  }
...
var extraInputs = params => {
  const changeThreshold = JSBI3.BigInt(`${params.changeThreshold}`)
  const feeRate = JSBI3.BigInt(`${params.feeRate}`)
  const currentChangeOutput = params.tx.outputs[params.tx.outputs.length - 1]
  const currentChange = JSBI3.BigInt(currentChangeOutput.capacity)
  const fee = JSBI3.BigInt(
    calculateTransactionFee(`0x${getTransactionSize(params.tx).toString(16)}`, `0x${feeRate.toString(16)}`),
  )
  const lack = JSBI3.subtract(JSBI3.add(fee, changeThreshold), currentChange)
  if (JSBI3.LE(lack, JSBI3.BigInt(0))) {
    return {
      ...params.tx,
      outputs: [
        ...
        {
          ...currentChangeOutput,
          capacity: `0x${JSBI3.subtract(currentChange, fee).toString(16)}`,
        },
      ],
    }
  }
  params.cells.sort((c1, c2) => +JSBI3.subtract(JSBI3.BigInt(c1.capacity), JSBI3.BigInt(c2.capacity)))
  const SIZE_PER_INPUT = JSBI3.BigInt(44)
  const FEE_PER_INPUT = JSBI3.divide(JSBI3.multiply(SIZE_PER_INPUT, feeRate), JSBI3.BigInt(1e3))
  for (let i = 1; i <= Math.min(params.extraCount, params.cells.length); i++) {
    const extraCost = JSBI3.multiply(JSBI3.BigInt(i), FEE_PER_INPUT)
    const totalLack = JSBI3.add(lack, extraCost)
    const extraCapacity = params.cells
      .slice(0, i)
      .reduce((sum, c) => JSBI3.add(sum, JSBI3.BigInt(c.capacity)), JSBI3.BigInt(0))
    if (JSBI3.GE(extraCapacity, totalLack)) {
      const inputs = [
        ...params.tx.inputs,
      ...
      const change = JSBI3.add(changeThreshold, JSBI3.subtract(extraCapacity, totalLack))
...
var calculateMaximumWithdraw = (outputCell, outputDataCapacity, depositDAO, withdrawDAO) => {
  ...
  const occupiedCapacity = JSBI4.asUintN(128, JSBI4.multiply(JSBI4.BigInt(1e8), JSBI4.BigInt(depositCellSerialized)))
  return `0x${JSBI4.add(
    JSBI4.divide(
      JSBI4.multiply(
        JSBI4.subtract(JSBI4.asUintN(128, JSBI4.BigInt(outputCell.capacity)), occupiedCapacity),
        JSBI4.asUintN(128, JSBI4.BigInt(extractDAOData(withdrawDAO).ar)),
      ),
      JSBI4.asUintN(128, JSBI4.BigInt(extractDAOData(depositDAO).ar)),
    ),
    occupiedCapacity,
  ).toString(16)}`
}
...
var formatter = {
  ...
  toPageSize: (pageSize = '0x32') => {
    const size = JSBI4.BigInt(`${pageSize}`)
    const MAX_SIZE = 50
    const MIN_SIZE = 0
    if (JSBI4.greaterThan(size, JSBI4.BigInt(MAX_SIZE))) throw new PageSizeTooLargeException(pageSize, MAX_SIZE)
    if (JSBI4.lessThan(size, JSBI4.BigInt(MIN_SIZE))) throw new PageSizeTooSmallException(pageSize, MIN_SIZE)
  ...
  constructor(node, options) {
    ...
    this.call = (...params) => {
      ...
      return axios({
        ...
        return __privateGet(this, _options).resultFormatters?.(res.data.result) ?? res.data.result
      })
    }
...
var getBigInts = ({ fee, capacityThreshold, changeThreshold }) => {
  ...
  return {
    targetFee: JSBI4.BigInt(`${fee}`),
    minCapacity: JSBI4.BigInt(`${capacityThreshold}`),
    minChange: JSBI4.BigInt(`${changeThreshold}`),
    zeroBigInt: JSBI4.BigInt(0),
  }
}
...
var getTargetOutputs = ({ outputs, minCapacity }) => {
  return outputs.map(output => {
    const capacity = JSBI4.BigInt(`${output.capacity}`)
    if (JSBI4.lessThan(capacity, minCapacity)) {
  ...
  for (let i = 0; i < inputScripts.length; i++) {
    ...
    for (let j = 0; j < unspentCells.length; j++) {
      ...
      if (!safeMode || (c.data === EMPTY_DATA && !c.type)) {
        ...
        sum = JSBI4.add(sum, JSBI4.BigInt(c.capacity))
        if (JSBI4.greaterThanOrEqual(sum, costCapacity)) {
          break
        }
      }
    }
    if (JSBI4.greaterThan(sum, costCapacity)) {
      break
    }
  }
  if (JSBI4.lessThan(sum, costCapacity)) {
...
var generateRawTransaction = ({
  ...
  const targetCapacity = targetOutputs.reduce((acc, o) => JSBI4.add(acc, o.capacity), zeroBigInt)
  const costCapacity = JSBI4.add(JSBI4.add(targetCapacity, targetFee), minChange)
  const changeOutput = {
    capacity: zeroBigInt,
  ...
  if (JSBI4.greaterThan(inputSum, JSBI4.add(targetCapacity, targetFee))) {
    changeOutput.capacity = JSBI4.subtract(JSBI4.subtract(inputSum, targetCapacity), targetFee)
  }
  const outputs = targetOutputs.map(o => ({ ...o, capacity: `0x${o.capacity.toString(16)}` }))
  if (JSBI4.greaterThan(changeOutput.capacity, zeroBigInt)) {
