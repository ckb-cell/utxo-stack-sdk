...
const { hashMultisig } = require('../dist/multisig')
...
const branch = new Branch('http://localhost:8114')
const multisigLockScript = {
  hashType: branch.utils.systemScripts.SECP256K1_MULTISIG.hashType,
  codeHash: branch.utils.systemScripts.SECP256K1_MULTISIG.codeHash,
  args: hashMultisig(multisigConfig),
}
const multisigLockHash = branch.utils.scriptToHash(multisigLockScript)
...
const tx = {
  cellDeps: [
    {
      outPoint: branch.utils.systemScripts.SECP256K1_MULTISIG.testnetOutPoint,
      depType: branch.utils.systemScripts.SECP256K1_MULTISIG.depType,
    },
  ],
  headerDeps: [],
  ...
  outputs: [
    {
      ...
      lock: {
        ...
        codeHash: branch.utils.systemScripts.SECP256K1_BLAKE160.codeHash,
        hashType: branch.utils.systemScripts.SECP256K1_BLAKE160.hashType,
      },
    },
    {
...
const transactionHash = branch.utils.rawTransactionToHash(tx)

// will be PartiallySigned after alice sign
const aliceSign = branch.signWitnesses(
  new Map([
    [
      multisigLockHash,
      {
...
const bobSign = branch.signWitnesses(
  new Map([
    [
      multisigLockHash,
      {
  ...
  (async function () {
    ...
    const txHash = await branch.rpc.sendTransaction(tx)
    console.log(txHash)
  })(),
)
