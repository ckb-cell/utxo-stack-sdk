...
const address = branch.utils.privateKeyToAddress(privateKey)
...
const unspentCells = [
  {
    ...
    outPoint: {
      txHash: '0xdf45112919d4af12b10fed94a93798ae6ddd8c89a11d90980022dcf695babca9',
      index: '0x0',
    },
...
const generateRawTransaction = async () => {
  ...
  const rawTx = branch.generateRawTransaction({
    fromAddress: address,
    toAddress: address,
  ...
  console.info(`outputs capacity: ${rawTx.outputs.map(output => BigInt(output.capacity))}`)
  console.groupEnd()
  return rawTx
}
...
const sendAllBalance = async () => {
  ...
  const signedTx = branch.signTransaction(privateKey)(rawTx)
